...
de colisão dos hádrons. Ainda, mesmo que partículas massivas sejam produzidas
...
\caption{Um esboço contendo exemplos de interação de partículas com os
...
produzidas não é suficiente para produzir novos pares. Os processos de mais
...
partículas do chuveiro reduza gradativamente.
...
eletromagnético de mesma energia. Esse fato tem como consequência a disposição
...
eletromagnéticos, fazendo inútil alta granularidade nos \gls{hcal}. 
Esse fato é muito explorada para realizar a discriminação entre as 
partículas que interagem com os calorímetros, buscando perfís de chuveiros
justos para partículas eletromagnéticas.
...
menor que \gls{lambdaint}, fato importante para minimizar a energia depositada
...
aproximadamente nulos. Para se identificar o \gls{ptmiss} é necessário em
primeiro lugar hermerticidade, de forma que ele seja capaz de
detectar os resíduos gerados pelas colisões. Aos calorímetros esse fato é
especialmente importante, uma vez que geralmente jatos hadrônicos
...
\caption{Os diversos calorimetros do ATLAS. Extraído de
...
\caption{Seção de corte longitudinal do ECAL. Extraído de
\cite{cal_tdr}}
...
\caption{Esboço da estrutura de acordeão e a granularidade de suas camadas  para
o barril. A quantidade de $X_0$ típicos para cada uma das camadas também são
...
Os requisitos de linearidade de resolução de energia devem atender aqueles
especificados para o \nameref{par:cal_tilecal}. Ainda, um tempo de pico de sinal
de $\sim40$ ns deve ser atendido pelo \gls{lar}. O número de \gls{lambdaint} do
\gls{hec} podem ser vistos na Figura~\ref{fig:cal_had_lambda}.
...
bóssom de Higgs é de $2<|\gls{eta}|<5$. Novamente veja a
...
cabos para o \gls{id}, e uma camada cintiladora é colocada entre os dois criostatos
com o objetivo de recuperar parte da energia perdida, principalmente por
partículas hadrônicas, mas também ajuda a reconstruir a energia de elétrons e
fótons. Os disposição desses dois calorímetros pode ser observada na
...
\caption{O Espectômetro de Múons e seus subsistemas: as câmaras medidoras de
...
fundamentais para o sucesso do experimento. Será realizada uma breve descrição
das infraestruturas utilizadas para que se tenha um conhecimento 
das ferramentas sem entrar em detalhes, uma vez que elas tem um cunho 
mais prático.

Infraestruturas \cite{root} (\emph{frameworks}), 
quando referidas em engenharia de software,
...
para fazer a reconstrução rodando os algoritmos tanto de análise em tempo real
quanto os de análise a posteriori, rodar simulações de Monte Carlo, digitalizações etc., 
...
\cite{grid}. A comunicação entre as infraestruturas citadas é realizada através do sistema
...
informações dos eventos de colisão em um registro chamado \emph{HepMC}.
...
\emph{Geant4} para descrever como estas partículas produzidas pelo Pythia interagem 
...
identificar e salvar informações geradas pelo mesmo no Storegate, de forma que
...
QCD\footnotemark estão desenhados em azul para o estado inicial e vermelho para o estado
...
\footnotetext{Os fenômenos de \emph{bremsstrahlung} de QCD são parecidos 
com os mesmos para QED, explicados no Tópico~{\ref{par:chuveiro_em}}, mas, ao invés
de partícula eletricamente carregadas, é correspondente para partículas contendo
cores, a carga correspondente para a força forte, mas liberando glúons ao invés
de fótons. Os glûons, diferente dos fótons, podem sofrer de seu próprio \emph{bremsstrahlung} 
pois eles carregam carga forte e gerar, assim, mais glûons} 
...
sequência de trabalhos até se gerar os dados para análise.

\subsubsection{Formatos dos dados}
\label{sssec:athena_dados}

Os diferentes formatos \cite{atlaswb} utilizados na sequências de trabalho do \emph{Athena}
são:
...
uma representação persistente dos dados de eventos;
...
\item \textbf{\gls{tag}}: são arquivos contendo eventos de física de
interesses da \gls{aod} rotulados, contendo $\sim$1 kB/evento;
\end{itemize}
...
\newacronym[type=Abrev]{d1pd}{\ensuremath{D^1PD}}{\emph{Primary Derived Physics Datasets}}
\newacronym[type=Abrev]{d2pd}{\ensuremath{D^2PD}}{\emph{Secondary Derived Physics Datasets}}
\newacronym[type=Abrev]{d3pd}{\ensuremath{D^3PD}}{\emph{Tertiary Derived Physics Datasets}}
...
Processador de Rotinas, Processamento Paralelo, \gls{rtti}, Comunicação em Network e Threads.
