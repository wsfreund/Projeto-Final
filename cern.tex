...
\caption[Estabilidade das partículas no feixe dos síncrotons]{Estabilidade das partículas no feixe dos síncrotons. No eixo y, a
...
%
    \end{center}
    \caption[A utilização do magneto de quadripólos na concentração das partículas
...
%
    \end{center}
    \caption[As diversas colisões possiveis na física de altas energias]{%
...
de colisão dos hádrons. Ainda, mesmo que partículas massiças sejam produzidas
...
espectômetro separado para múons. O \gls{cms} não possui tal
...
a estudar sinais hadrônicos e elétrons, e o espêctometro de múons avançado,
...
facilidade, sendo somente detectados pelo Espectômetro de Múons. Léptons Neutrinos não
são detectados por nenhum dos subdetectores do ATLAS, e por esse motivo é
necessário hermerticidade para encontrar falhas na conservação de momento
...
\gls{hcal} e Espectômetro de Múons. De maneira resumida, o \gls{id} identifica a
...
Toróide}
\newacronym[type=Abrev]{bt}{BT}{Barril do Toróide}
...
e fornecem o campo magnético necessário para o Espectômetro de Múons como grandes toróides 
...
em colisão é despresível, de forma que o \acrlong{pt}
...
%        }\hspace{0.01\textwidth}
        \subfigure[Seção de corte do Detector Interno mostrando seus subsitemas. Extraído de
\cite{paper_atlas}.  ]{%
            \label{fig:id_secao}
...
nele utilizados. As especificações do \gls{ecal} e \gls{hcal} serão análisadas em seguida, 
...
calorimetria uma vez que algumas noções são de grande importancia para o 
entendimento do trabalho e do algorítmo discriminador proposto.
...
Os calorímetros podem ser de dois tipos, homogênios e de amostragem. 
...
Calorímetros homogênios utilizam um único material para realizar as duas
...
forma a excitar o meio ou aquece-lo. O tipo de interação ocorrida depende da
...
podem ocorrer tanto através de partículas elétricamente carregadas, 
...
moléculas sem necessáriamente ionizá-las. Os átomos em estado excitado são
...
hadrônicas por elas serem compostas de sistemas massiços. Ao mesmo tempo, 
...
massiços que elétrons (sofrem menor efeito de \emph{bremsstrahlung}),
e por isso são utilizados o Espectômetro de Múons para
...
indiretamente através dos resíduos de seu decaímento. Com isso, apenas elétrons,
pósitrons e fótons são detectados no \gls{ecal}, mas valhe ter em mente que eles
...
separar um jato hadrônico de um decaímento duplo em jatos próximos, sendo
...
entre as partículas que interagem com os calorímetros, buscando perfís de chuveiros
...
por hadrôns no \gls{ecal}. Ainda, é necessário uma granularidade 
...
primeiro lugar hermerticidade, de forma que ele seja capaz de
...
\newacronym[type=Abrev]{lar}{LAr}{Argônico Líquido}
...
O sistema de calorímetria \cite{cal_tdr,ecal_tdr,hcal_tdr} do \gls{atlas}, retratados na
...
Os diversos calorimetros do ATLAS. Extraído de
...
das camadas de segmentação longitudinais, o que ocorre devido a expanção da
...
intrinsicos, assim como sua estabilidade de resposta temporal. O sinal é gerado
...
na parte frontal e trazeira. Essa estrutura pode ser visualizada na Figura~\ref{fig:estr_acordeao}
...
calorimetro, existem três regiões de fendas, também simétricas em \gls{phi}, 
...
\caption[Esboço da escrutura do acordeão e a granularidade de suas camadas para
...
Além disso, a partícula $\pi^0$ é instavel e decai de acordo com
...
poucos milimetros, uma vez que só assim será possível fazer a distinção dos dois
...
energia.
\item \textbf{Camada Trazeira (\emph{Back})}: a terceira camada tem a mesma
...
maiores a 24 \gls{X0} no barríl, e maiores a 26 \gls{X0} na tampa. Os valores de \gls{X0} em 
...
\newacronym[type=Abrev,\glslongpluralkey={Multiplcadores de Fótons}]{pmt}{PMT}{Multiplicador de Fótons}
...
energia e reduzir o ruído causado no Espectômetro de Múons. Os valores de
...
\gls{hec} contendo cada uma 32 módulos identicos. Cada módulo se consiste de 24
...
para aguentar os elevados indices de radiação próximos ao tubo do feixe. Em sua primeira camada, 
é utilizada uma matriz de metal absorvedora de cobre contendo buracos igualmente
distribuidos. Nesses buracos são colocados uma estrutura de hastes coaxiais e tubos, 
...
enquanto o espaço remanscente entre o tubo e a haste é preenchido por \gls{lar},
...
O material das hastes e da matriz são substituidos de cobre para tugstênio
...
%
    \end{center}
    \caption[As diferentes estruturas dos subsitemas de calorimetria do ATLAS]{%
As diferentes estruturas dos subsitemas de calorimetria do ATLAS.
     }%
\end{figure}
...
constituido apenas de uma fina camada de \gls{lar} na região de
...
Na região de transição entre o barríl e a tampa, para
...
\paragraph{Sistema de Criogênia}
...
são feitas de aluminio. O criostato no barril envolve apenas o
...
estrutura. Parte da estrutura de Criogênia pode ser vista na
Figura~\ref{fig:secao_em_calo}.


\subsection{O Espectômetro de Múons}
...
de Múons \cite{muon_tdr} é o subdetector mais externo do \gls{atlas}. Diferente dos calorimetros
que empregam um processo de medição destrutivo, o Espectômetro de Múons,
...
\caption[O Espectômetro de Múons]{
O Espectômetro de Múons e seus subsistemas: as câmaras medidoras de
...
e tampa, respectivamente. A segunda e terceira letras referênciam para a camada
...
tipos, onde os múons que passam por uma fenda de gás entre um anôdo e catodo (por
...
Graças a grande dimensão do Espectômetro de Múon não é possível de firmar as
...
sistemas opticos de alinhamento, de forma que deslocamentos dentre $\sim$30
...
transpartes para o usúario de forma que seria impossível citar todas as 
...
enquanto o \emph{ROOT} é um ambiente dedicado a analise de dados. 
...
\gls{panda}, que não será descrito nesse trabalho por sair muito do escôpo, 
...
de fótons. Os glûons, diferente dos fótons, podem sofrer de seu próprio
\emph{bremsstrahlung}}}
...
\item \textbf{\gls{sg}}: é um ambiente que permite a propagacão de dados entre pacotes do
...
Essa infraestrtura é utilizada como plataforma de emulação do Sistema de
Filtragem e o Sistema de Reconstrução do \gls{atlas}, contendo diversas
distribuições. Cada distribuição do \emph{Athena} tem dependencias diferentes
pacotes de algoritmos, focando em uma funcionalidade. Por exemplo, a
distribuição \emph{AtlasOffline} é utilizada pelos físicos para fazerem análises
independentes; o \emph{AtlasProduction} é utilizado para
fazer as reconstruções oficiais da Colaboração; e o \emph{AtlasP1HLT} é a
distribuição que é utilizada nas máquinas do \gls{ip}1 para fazerem as análises 
do alto nível do Sistema de Filtragem. 
...
Normalmente esse conjunto de configurações necessárias para rodar o Athena são 
...
reais armazenados após passarem o Sistema de Filtragem. Serão realizadas uma
cadeia de trabalhos até se gerar os dados para análise. Para facilitar os
diversos passos da cadeia de reconstrução, um mecânismo de autoconfiguração e 
\cite{recotrf} está disponível para a colaboração.
...
\newacronym[type=Abrev]{bs}{BS}{\emph{Bytestream}}
\newacronym[type=Abrev]{rdo}{RDO}{\emph{Raw Data Object}}
...
binário\footnote{Por isso muitas vezes são chamados também de \emph{RAW data},
ou dados crus.};
\item \textbf{\gls{rdo}}: assim como o \gls{bs}, contêm uma representação persistente 
dos dados de eventos, mas em objetos de C++. Elas geralmente são geradas de
digitalizações dos G4 \emph{Hits} quando realizando a reconstrução de dados de
simulação, mas também podem ser geradas de \gls{bs}, embora não seja usual;
...
calibração que podem ser executadas novamente através desses dados, entretanto
nem toda a informação da \gls{rdo} está presente. A
...
\item \textbf{\gls{aod}}: parecidos com os arquivos de \gls{esd}, mas contêm
...
interesses da \gls{aod} rotulados em uma estrutura de árvores do \emph{ROOT}, 
contendo cerca de $\sim$1 kB/evento;
\end{itemize}
...
\gls{aod}/\gls{tag} é possível gerar arquivos de análise com formatos 
\emph{ROOT}/\acrshort{pool} (um projeto para unificar os formatos de
...
tão simples. A \gls{cbnta} é um arquivo \emph{ROOT};
\item \textbf{\gls{dpd}}: são arquivos persistentes no formato \gls{pool}. Eles
podem ser de três tipos diferentes: primários, secundários, ou terceários:
\begin{itemize}
\item \textbf{\gls{d1pd}}: basicamente tem o mesmo formato que as \glspl{aod},
mas menores, contendo cerca de 10 kB/evento. Nelas são permitidas operações para
...
\end{itemize}
As \glspl{d1pd} derivadas de \glspl{esd} são chamadas de \acrshortpl{desd}, e o
análogo para as \acrshortpl{daod};
...
\acrshortpl{d2esd}, e aqueles provenientes de \glspl{aod} chamados de
\acrshortpl{d2aod};
...
Estima-se que o \gls{lhc} irá gerar, para todos os seus experimentos, cerca de
15 PB/ano, que milhares de cientistas irão acessar e analisar. O objetivo do
\gls{lcg} é construir e manter uma infraestrutura de análise para toda a
comunidade de física de altas energias do \gls{lhc}.

O projeto utiliza uma grade para fornecer suporte computacional aos experimentos
em quatro camadas. As especificações das camadas são
\cite{grid,atlas_computing_tdr}:

\begin{itemize}
\item Camada-0 (\emph{Tier-0}): os arquivos brutos selecionados pelo Sistema de
Filtragem do \gls{atlas} serão gravados na Camada-0. O primeiro passo de
reconstrução será realizado por essa camada, onde uma copia dos dados
reconstruidos
(\gls{rdo}/\gls{bs}$\rightarrow$\gls{esd}/\gls{aod}/\gls{tag}/\acrshort{desd}) 
serão gravados em disco local. Uma copia dos arquivos \glspl{bs}
serão distribuídas entre os centros da Camada-1 relacionados com o experimento,
cerca de 10 deles servem o \gls{atlas}.
...
quantidades de dados. Os dados mais recentes estarão escritos em disco, e os com
maior latência estarão, talvez, em fita. Muitos dos dados utilizados neste trabalho estão
armazenados nesta camada. No total existem 11 centros definidos, em maioria,
servindo a vários dos experimentos;
\item Camada-2 (\emph{Tier-2}): provêm capacidade computacional e capacidade de
...
laboratórios podem fazer parte do processamento e análise dos dados do \gls{lhc}
como parte da Camada-3. Eles saem do escôpo do projeto \gls{lcg}, contudo é
